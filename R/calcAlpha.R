...
calcAlpha <- function(asvtable = NULL, metadata = NULL, numRare = 0, method = "standard", minReads = 1) {
  ...
  calculations <- function(df) {
    all <- df %>%
      ...
      dplyr::summarise(
        Sprichness = vegan::specnumber(.data$value),
  ...
  if (numRare > 0) { # doing a bunch of lapply's is dumb and inelegant, but whatever, it works for now. may clean up later when i have more time.
    ...
    rarefactions3 <- lapply(rarefactions2, function(df) {
      df[, names(df) != "SampleID"]
    })
    rarefactions4 <- lapply(rarefactions3, function(df) {
      dplyr::bind_cols(asvtable[1], df)
    })
    rarefactions5 <- lapply(rarefactions4, function(df) {
      tidyr::pivot_longer(df, cols = !c(.data$SampleID), names_to = "ASV", values_to = "value")
    })
    rarefactions6 <- lapply(rarefactions5, calculations)
    out <- dplyr::bind_rows(rarefactions6) %>%
      dplyr::group_by(.data$SampleID) %>%
      dplyr::summarise(
        Sprichness = mean(.data$Sprichness, na.rm = TRUE),
