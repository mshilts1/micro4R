...
calcBeta <- function(asvtable = NULL, metadata = NULL, numRare = 400, method = "bray", category = NULL, minReads = 1000, makeFigs = FALSE, ...) {
  ...
  if (makeFigs == TRUE) {
    ...
    print(ggplot(pcoa_metadata, aes(x = .data$PCoA1, y = .data$PCoA2)) +
      geom_point(shape = 21, size = 3, alpha = 0.7, aes(fill = .data[[category]])) +
      stat_ellipse(geom = "polygon", alpha = 0.2, level = 0.9, aes(colour = .data[[category]], fill = .data[[category]])) +
      geom_point(data = centroids, size = 5, shape = 23, aes(x = .data$PCoA1, y = .data$PCoA2, fill = .data$grps)) +
      ...
      xlab("PCoA 1") +
      ylab("PCoA 2")) #+ facet_wrap(~.data[[category]])
    dev.off()
    ...
    print(ggplot(eigtab_l, aes(x = .data$Var2, y = .data$value)) +
